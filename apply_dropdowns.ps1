...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="${ns}">
   ...
   <soapenv:Body><tns:${reqName}/></soapenv:Body>
            ...
            try {
                ...
                console.error("Failed to fetch cross-service options. Ensure you are accessing this UI via the API Gateway.", e); 
                return []; 
            }
        }
            ...
            for (let f of CFG.fields) {
                ...
                if (f.type === 'select') {
                    group.innerHTML = `
                        ...
                        <select id="${f.name}" name="${f.name}" required style="width: 100%; padding: 0.8rem 1rem; background: rgba(15, 23, 42, 0.6); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 8px; color: white; outline: none; appearance: none;">
                    ...
                    fetchOptions(f.sourceService, f.sourceReq, f.sourceNs, f.sourceTags, f.sourceId, f.sourceLabel).then(options => {
                        ...
                        if(options.length === 0) {
                            select.innerHTML = `<option value="">No data found (Use API Gateway)</option>`;
                        }
                    });
                } else {
