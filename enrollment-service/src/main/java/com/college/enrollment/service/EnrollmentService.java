...
public class EnrollmentService {
    ...
    public EnrollmentService(EnrollmentRepository enrollmentRepository, Validator validator, RemoteSoapService remoteSoapService) {
    ...
    public Enrollment createEnrollment(Enrollment enrollment) {
        ...
        try {
            ...
            throw new DuplicateEnrollmentException("Duplicate enrollment data");
        } catch (DataAccessException exception) {
            throw new EnrollmentDatabaseException("Unable to save enrollment", exception);
        }
    }
    ...
    public Enrollment getById(Long id) {
        ...
        try {
            ...
            throw new EnrollmentDatabaseException("Unable to retrieve enrollment", exception);
        }
    }
    ...
    public List<Enrollment> getAllEnrollments() {
        try {
            ...
            throw new EnrollmentDatabaseException("Unable to retrieve enrollments", exception);
        }
    }
    ...
    public Enrollment updateEnrollment(Long id, Enrollment updatedEnrollment) {
        ...
        if (!existing.getEnrollmentCode().equalsIgnoreCase(updatedEnrollment.getEnrollmentCode())
        ...
        boolean studentChanged = !existing.getStudentId().equals(updatedEnrollment.getStudentId());
        boolean courseChanged = !existing.getCourseId().equals(updatedEnrollment.getCourseId());

        if (studentChanged || courseChanged) {
            if (enrollmentRepository.existsByStudentIdAndCourseId(updatedEnrollment.getStudentId(), updatedEnrollment.getCourseId())) {
                throw new DuplicateEnrollmentException("Student is already enrolled in this course");
            }
            remoteSoapService.validateStudentExists(updatedEnrollment.getStudentId());
            remoteSoapService.validateCourseExists(updatedEnrollment.getCourseId());
        }

        existing.setEnrollmentCode(updatedEnrollment.getEnrollmentCode());
        ...
        existing.setAcademicYear(updatedEnrollment.getAcademicYear());
        existing.setEnrollmentType(updatedEnrollment.getEnrollmentType());
        ...
        try {
            ...
            throw new EnrollmentDatabaseException("Unable to update enrollment", exception);
        }
    }
    ...
    public void deleteEnrollment(Long id) {
        ...
        try {
            ...
            throw new EnrollmentDatabaseException("Unable to delete enrollment", exception);
        }
    }

    private void validateEnrollment(Enrollment enrollment) {
        if (enrollment == null) {
            throw new EnrollmentValidationException("Enrollment data cannot be null");
        }
        Set<ConstraintViolation<Enrollment>> violations = validator.validate(enrollment);
        if (!violations.isEmpty()) {
            StringBuilder message = new StringBuilder("Validation failed: ");
            for (ConstraintViolation<Enrollment> violation : violations) {
                message.append(violation.getPropertyPath()).append(": ").append(violation.getMessage()).append("; ");
            }
            throw new EnrollmentValidationException(message.toString());
        }
    }

    private void validateId(Long id) {
        if (id == null || id <= 0) {
            throw new EnrollmentValidationException("Enrollment ID must be a positive number");
        }
    }

    private void normalize(Enrollment enrollment) {
        if (enrollment.getEnrollmentCode() != null) {
            enrollment.setEnrollmentCode(enrollment.getEnrollmentCode().trim().toUpperCase());
        }
        if (enrollment.getStatus() != null) {
            enrollment.setStatus(enrollment.getStatus().trim());
        }
        if (enrollment.getAcademicYear() != null) {
            enrollment.setAcademicYear(enrollment.getAcademicYear().trim());
        }
        if (enrollment.getEnrollmentType() != null) {
            enrollment.setEnrollmentType(enrollment.getEnrollmentType().trim());
        }
    }
}