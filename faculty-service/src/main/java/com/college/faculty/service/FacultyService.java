...
public class FacultyService {
    ...
    public Faculty createFaculty(Faculty faculty) {
        ...
        if (facultyRepository.existsByEmployeeNumber(
            ...
            throw new DuplicateFacultyException(
    ...
    public Faculty updateFaculty(
        ...
        if (!existingFaculty.getEmployeeNumber()
                .equals(updatedFaculty.getEmployeeNumber())
                && facultyRepository.existsByEmployeeNumber(
                updatedFaculty.getEmployeeNumber())) {
        ...
        if (!existingFaculty.getEmail()
                .equalsIgnoreCase(updatedFaculty.getEmail())
                && facultyRepository.existsByEmail(
        ...
        existingFaculty.setEmployeeNumber(
                updatedFaculty.getEmployeeNumber()
        );

        existingFaculty.setFirstName(
                updatedFaculty.getFirstName()
        );

        existingFaculty.setLastName(
                updatedFaculty.getLastName()
        );
        ...
        existingFaculty.setPhone(
                updatedFaculty.getPhone()
        );
        ...
        existingFaculty.setDesignation(
                updatedFaculty.getDesignation()
        );
    ...
    private void validateFaculty(Faculty faculty) {
        ...
        if (faculty == null) {
            ...
            throw new FacultyValidationException(
                    "Faculty data cannot be null"
            );
        }
        ...
        if (!violations.isEmpty()) {
            ...
            for (ConstraintViolation<Faculty> violation
                    : violations) {
    ...
    private void normalizeFaculty(Faculty faculty) {
        ...
        if (faculty.getEmployeeNumber() != null) {
            faculty.setEmployeeNumber(
                    faculty.getEmployeeNumber().trim()
            );
        }
        ...
        if (faculty.getEmail() != null) {
            faculty.setEmail(
                    faculty.getEmail().trim().toLowerCase()
            );
        }
